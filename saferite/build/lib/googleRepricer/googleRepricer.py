...
access_key = 'AKIAJMA7KVN7QTO7U7QQ'
marketplace_id = 'ATVPDKIKX0DER'
seller_id = 'A289SXMG0BFOA0'
secret_key = '0Q/6hGGvwxxAoOhDP1OfHPQmeQmvnVkEkJDjIDui'

col2 = {'Sold by':["NO"], 'Details & special offers':["NO"], 'Item price':["NO"],
   'Unnamed: 7':["NO"], 'Unnamed: 9':["NO"], 'Unnamed: 12':["NO"]}

col = ['Sold by', 'Details & special offers', 'Item price',
   'Unnamed: 7', 'Unnamed: 9', 'Unnamed: 12']

def shopping_bot(shop_id):
	url = 'view-source:https://www.google.com/shopping/product/' + str(shop_id) + '/online?prds=scoring:tp'
	...
	page = soup.find("div",{"class":"sg-product__dpdp-c"})
...
def fix_data_repricer(page, shop_id, bc_id, amazon_price, amazon_vendor):
	...
	final_table['Discount'] = np.where(final_table['Details & special offers'].str.contains("Off"),"Y","N")
	try:
		final_table['Arrival Date'] = np.vectorize(find_arrival)(final_table['Details & special offers'], 'Arrives')
	except:
		final_table['Arrival Date'] = ""
	try:
		final_table['Sold by'] = np.vectorize(find_arrival)(final_table['Sold by'], '%')
		final_table['Sold by'] = np.vectorize(find_arrival)(final_table['Sold by'], 'Show')
	except:
		pass
	try:
		final_table['Arrival Date'] = final_table['Arrival Date'].str.replace('–','-')
	except:
		pass
	...
	data = {
	"BC ID": int(bc_id),
	"date": dt.datetime.today().strftime('%m-%d-%Y %H:%M'),
	"Shopping ID": str(shop_id),
	"Amazon vendor": amazon_vendor,
	"Amazon Price": amazon_price,
	"Google Shopping": sellers_data
	}
	return data
...
def fix_data_mongo(dataFrame):
	...
	result['Total cost'] = result['Total cost'].str.replace('$','')
	result['Total cost'] = result['Total cost'].str.replace('NO','')
	result['Total cost'] = result['Total cost'].str.replace('\+Shipping','')
	result['Total cost'] = result['Total cost'].str.replace(',','')
	result['Total cost'] = result['Total cost'].str.replace('\+Tax','')
	...
	result['Item price'] = result['Item price'].apply(pd.to_numeric)
	result['Total cost'] = result['Total cost'].apply(pd.to_numeric)
	...
	result['Amazon Price'].fillna(9999999, inplace=True)
...
for i in range(len(data_after['Shopping ID'])):
    try:
        page = shopping_bot(data_after['Shopping ID'][i])
        if not data_after['asin'][i] == 0:
            try:
