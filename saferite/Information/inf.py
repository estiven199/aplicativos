...
class information():
    ...
    z = zohoapi_new.BooksZohoApi(zoho_token['BOOKS_KEY'])
    b = bcispapi()
    ...
    def update_Warehouse(self,i,data,unique,value):
        r3 = self.z.get_sorder(data['salesorder_id'][0])
        items = []
        ...
        update_data = {'salesorder_number':order,
                      ...
                      'line_items': items, }
        orders_update = self.z.update_sorder(str(unique['salesorder_id'][i]),update_data)
    ...
    def create_shipment(self,carrier,trackig,i,unique,value_id,r1=None):
        ...
        if value_id == 'Create':
            ...
            if status == 'Delivered':
                r = self.z.mark_shipment_as_delivered(str(shipment))
                print(r['message'])
                value  = 1
            ...
            if status == 'Delivered':
                r = self.z.mark_shipment_as_delivered(str(unique['shipment_id'][i]))
                print(r['message'])
                value  = 1
